...
export function OrkutNostalgicIconSet(props) {
  const orkutDocs = props.orkutDocs;
  const trustCoolSexyCounter = props.trustCoolSexyCounter;
  return (
    <OrkutNostalgicIconSet.List>
      {orkutDocs.map(({ name, slug, icon }) => (
        <li key={`orkut__icon_set__${name}_${slug}`}>
          ...
          <img key={`orkut__icon_set__${name}_${slug}_img`} className="OrkutNostalgicIconSet__iconSample" src={`https://alurakut.vercel.app/icons/${icon}.svg`} />
            {slug ? slug : 0}
          </span>
        </li>
      ))}
      {trustCoolSexyCounter.map(({ name, slug, icon }) => {
        const total = slug ? slug : 2;
        return (
